...
def select_apr():
    ...
    WAIT(0.5)
    operation_mode(secod_option=lambda: BrakeWindow("Espere a seleção ser concluída e aperte Entendi").show())
    ...
    KeyboardBasicFunctions._write_text(f'Àrea da Propriedade Rural - A.P.R',speed=0.002)
    ...
    KeyboardBasicFunctions._press_enter()
    WAIT(1)
    area_influencia_direta_coordenada = get_or_set_coordinate(objeto_id=19,mensagem="Clique na área de influência direta")
    click(area_influencia_direta_coordenada[0],area_influencia_direta_coordenada[1],button_side="right")
    WAIT(0.2)
    ArcGISKeyboardFunctions._zoom_to_layer()