...
def build_squares():
    ...
    for position,type in zip(position_list,Text_infos.current_items):
        ...
        WAIT(0.2)
        click(symbol_coordinates[0],symbol_coordinates[1],wait_time=0.1, ammount_click=3)
        fill_color_coordinates =  get_or_set_coordinate(6,"Clique em 'fill color'")
        click(symbol_coordinates[0],symbol_coordinates[1],wait_time=0.1)
        click(fill_color_coordinates[0],fill_color_coordinates[1],wait_time=0.1)
        ArrowsKeyboard._press_up(2)
        WAIT(0.5)
        KeyboardBasicFunctions._press_enter()
        KeyboardBasicFunctions._write_text(str(current_style[type][0]))
        KeyboardBasicFunctions._press_tab(wait_time=0.1)
        KeyboardBasicFunctions._write_text(str(current_style[type][1]))
        KeyboardBasicFunctions._press_tab(wait_time=0.1)
        KeyboardBasicFunctions._write_text(str(current_style[type][2]))
        KeyboardBasicFunctions._press_enter()
        outline_cordenadas =  get_or_set_coordinate(7,"Clique em 'fill color'")
        click(symbol_cordenadas[0],symbol_cordenadas[1],wait_time=0.1)
        click(outline_cordenadas[0],outline_cordenadas[1],wait_time=0.1)
        ArrowsKeyboard._press_up(1)
        WAIT(0.5)
        KeyboardBasicFunctions._press_enter()
        KeyboardBasicFunctions._press_tab(wait_time=0.1)
        KeyboardBasicFunctions._write_text("0,40")
        click(symbol_cordenadas[0],symbol_cordenadas[1],wait_time=0.1, ammount_click=3)
        ...
        KeyboardBasicFunctions._press_tab(wait_time=0.1)
        KeyboardBasicFunctions._write_text(str(x_size_square))
        KeyboardBasicFunctions._press_tab(wait_time=0.1)
        KeyboardBasicFunctions._write_text(str(y_size_square))
        WAIT(0.5)
        KeyboardBasicFunctions._press_enter()
